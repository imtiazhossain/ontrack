...
export async function resolveTravelInvite(
  ...
  const { data, error } = await requireInviteClient()
    .from('travel_invites')
    .select('payload')
    .eq('code', value.slice(SHORT_INVITE_PREFIX.length))
